...
const stats = computed(() => {
    ...
    const overdue = todoState.value.todos.filter((todo) => !todo.completed && todo.dueDate && new Date(todo.dueDate) < new Date()).length;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-4xl space-y-6">
            ...
            <div class="rounded-lg border bg-white p-6 shadow-sm">
                <h2 class="mb-4 text-lg font-semibold">Add New Todo</h2>
                <div class="space-y-4">
                    <div>
                        <input
                            v-model="newTodoText"
                            ...
                            class="w-full rounded-md border border-gray-300 px-4 py-3 text-lg outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
                            :disabled="isLoading"
                        />
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class="w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
                                :disabled="isLoading"
                                list="categories"
                            />
            ...
            <!-- Stats -->
            <div v-if="stats.total > 0" class="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <!-- Filters & Sort -->
            <div v-if="todoState.todos.length > 0" class="rounded-lg border bg-white p-4 shadow-sm">
            ...
            <div class="rounded-lg border bg-white shadow-sm">
                <div v-if="todoState.todos.length === 0" class="p-12 text-center text-gray-500">
                    ...
                    <h3 class="mb-2 text-lg font-medium text-gray-900">No todos yet</h3>
                ...
                <div v-else class="divide-y">
                    <div
                        ...
                        :class="[
                            ...
                            isOverdue(todo) ? 'border-l-4 border-red-400 bg-red-50' : '',
                            isDueToday(todo.dueDate) && !todo.completed ? 'border-l-4 border-yellow-400 bg-yellow-50' : '',
                        ]"
                    >
                        <div class="flex items-start gap-3">
                            <button
                                ...
                                :disabled="isLoading"
                            >
                                <svg v-if="todo.completed" class="h-3 w-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                                    <path
                                        fill-rule="evenodd"
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="mb-1 flex items-center gap-2">
                                    <span :class="['font-medium', todo.completed ? 'text-gray-500 line-through' : 'text-gray-900']">
                                        {{ todo.text }}
                                    </span>

                                    <span :class="['rounded-full border px-2 py-0.5 text-xs font-medium', getPriorityColor(todo.priority)]">
                                        {{ todo.priority }}
                                    </span>
                                </div>
                                ...
                                <div class="flex items-center gap-4 text-sm text-gray-500">
                                    ...
                                    <span v-if="todo.dueDate" :class="isOverdue(todo) ? 'font-medium text-red-600' : ''">
                            ...
                            <button
                                ...
                                class="px-2 py-1 text-sm font-medium text-red-600 transition-colors hover:text-red-800"
                                :disabled="isLoading"
                            >
                                Delete
                            </button>
                ...
                <!-- Actions -->
                <div v-if="stats.completed > 0" class="border-t bg-gray-50 p-4">
                    <button
                        @click="clearCompleted"
                        :disabled="isLoading"
                        class="text-sm font-medium text-red-600 transition-colors hover:text-red-800 disabled:cursor-not-allowed disabled:opacity-50"
                    >
